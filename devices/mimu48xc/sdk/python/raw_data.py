...
MAX_FREQ = 562.5                                  # Max data rate transmission
...
out_rate = float(281.25)                        # Data rate (Hz) Note: Maximum supported data rate is 281.25 Hz when plot_graph is set to 0,
...
conn_type = 'usb'
plot_graph = 0                                  # Whether graph is required or not 1 else 0
...
def getData(pkt_data, num_imu, no_log):
    global scale_acc, scale_gyro, scale_magn
...
def insert_data_logfile(filedata, queue, num_imu, imu_item, fuse_item, temp_file, select_acm, binary_string):
    ...
    print "start logfile"
    global isRunning, isThreadRunning, isComplete
...
if d_graph == "OFF LINE" and pkt > 0:  # to display the graph off line
