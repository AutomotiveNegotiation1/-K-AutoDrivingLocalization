...
% aa = rosbag('global_cf_track3_noImage.bag');
aa = rosbag('2024-08-06-16-43-08.bag');
...
for kkg = 2 : length(IMGMsg)
    ...
    Eul0(kkg,:) = equPlane(Temp);
    Eul1(kkg,:) = quat2eul(Temp);
    

    % RotMat_diff = (eulr2dcm(EulTemp(kkg,:)))/(eulr2dcm(EulTemp(kkg-1,:)));
...
% 
figure(2314);hold off;plot(Eul0(:,1),'r.');hold on;plot(Eul0(:,2),'g.');plot(Eul0(:,3),'b.');
...
figure(12137);hold off;plot(EulTemp(:,1),'r.');hold on;plot(EulTemp(:,2),'g.');plot(EulTemp(:,3),'b.');

figure(12139);hold off;plot(Eul0(:,1),'r.');hold on;plot(Eul0(:,2),'g.');plot(Eul0(:,3),'b.');

figure(12140);hold off;plot(Eul1(:,1),'r.');hold on;plot(Eul1(:,2),'g.');plot(Eul1(:,3),'b.');
