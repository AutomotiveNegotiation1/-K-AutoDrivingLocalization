...
tempB = checkSametiem(tempB,SLAMtime);
tempC = checkSametiem(tempC,PosHUWBtime);

[vals,indxs] = max([SLAMtime(tempB(1)),PosHUWBtime(tempC(1))+TimeLag]);
TimeMin = vals;
[vals,indxs] = min([SLAMtime(tempB(end)),PosHUWBtime(tempC(end))+TimeLag]);
TimeMax = vals;

% [LsI] = min(find(SLAMtime(tempB(1)) < (PosHUWBtime(tempC)+TimeLag)));    % LsI~LfI  : tempB에 포함되는 tempC의 인덱스
% [LfI] = max(find(SLAMtime(tempB(end)) > (PosHUWBtime(tempC)+TimeLag)));
% [SsI] = min(find(PosHUWBtime(tempC(1))+TimeLag < (SLAMtime(tempB))));    % SsI~SfI  : tempC에 포함되는 tempB의 인덱스
% [SfI] = max(find(PosHUWBtime(tempC(end))+TimeLag > (SLAMtime(tempB))));
% 
% PosHUWBtime(tempC(LsI))
% Desa = find(PosHUWBtime(tempC)+TimeLag<PosHUWBtime(tempC(LsI)));
% 
% if length(Desa)~=0
%     TimeIndx = [PosHUWBtime(tempC(Desa(end)+1))+TimeLag:1e-3:PosHUWBtime(tempC(LfI))+TimeLag];
% else
%     TimeIndx = [PosHUWBtime(tempC(1))+TimeLag:1e-3:PosHUWBtime(tempC(LfI))+TimeLag];
% end

% TimeIndx = TimeIndx(101:end-100);

TimeIndx = [TimeMin+1e-3:1e-3:TimeMax-1e-3];
...
save('MapParams0820_chungra.mat','sA','s','roll','pitch','yaw','SLAMposInit');