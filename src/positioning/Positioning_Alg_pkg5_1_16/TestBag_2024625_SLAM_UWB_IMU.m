...
%aa = rosbag('2024-08-20-13-57-45.bag');
aa = rosbag('bag2_pkg_5_1_13_2024-08-20-16-32-43.bag');
...
for kkg = 1 : length(IMGMsg)
    ...
    IMGHeading(kkg,:) = [IMGMsg{kkg}.Pose.Orientation.W IMGMsg{kkg}.Pose.Orientation.X IMGMsg{kkg}.Pose.Orientation.Y IMGMsg{kkg}.Pose.Orientation.Z ];
...
while(length(UWBMsg{1})>=k0(1) || length(UWBMsg{2})>=k0(2) || length(UWBMsg{3})>=k0(3) || length(UWBMsg{4})>=k0(4) || length(gps_data_d)>=k0(6) || size(IMGpos,1)>=k0(7)   )
    ...
    % add map parameters to original PositionVector 
    %------------------------------------------------(changed by joo)
    s = 3.7138;
    % sA = [-0.035 -3.701 0.0456; 3.6505 0.1008 -0.0801; 0.0216 0.0121 0.9997];
    sA_00 = -0.035; 
    sA_01 = -3.701; 
    sA_02 = 0.0456; 
    ...
    pitch = 4.722; 
    yaw = -0.0216; 
    %SLAMposInit = [3.9997; 77.71; 0.0];
    SLAMposInit_0 = 3.9997;
    SLAMposInit_1 = 77.71; 
    SLAMposInit_2 = 0.0; 
    %-------------------------------------------------
    ...
    PositionOut = PositioningSystem_V5_1(PositionVector, MapParam);
    ...
    if GraphEvery == 1
        if (PosH(k) ~=0)
            ...
            % 
            % hold on;plot(real(arrowHeading) + Scale*real(PosH(k)), ...
            %              imag((arrowHeading)) + (size(ParkingLot,2)-Scale*imag(PosH(k))-Scale*Scale_global));

            %% Incheon Chungra
            Scale_chungra = 16.5;

            xarrow = -1*Scale_chungra*[-0.8 0.8 0 -0.8];
            yarrow = -1*Scale_chungra*[-1 -1 1 -1];
            arrowc = xarrow+j*yarrow;
            arrowHeading = arrowc*exp(j*(HeadingH(k)));

            hold on;plot(real(arrowHeading) + Scale_chungra *real(PosH(k)) + 150, ...
                         imag(arrowHeading) + (size(ParkingLot,2) - Scale_chungra *imag(PosH(k)) + 4.3*Scale_chungra * 11), 'r.');
