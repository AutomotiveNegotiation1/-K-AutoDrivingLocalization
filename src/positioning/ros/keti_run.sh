...
function start_imu()
{
    echo START IMU
    source devel/setup.bash 
    rosrun ublox_f9r ublox_f9r_init_node.py &
}
...
function start_uwb()
{
    echo START UWB start
    rosrun decawave_uwb mdek1001_init_node.py &
}

function start_basic_localizer()
{
    echo START LOCALIZER 
    rosrun utils location_drawer.py &
}

function start_ekf_localizer()
{
    echo START EKF LOCALIZER 
    rosrun utils ekf_uwb_localization.py &
}
