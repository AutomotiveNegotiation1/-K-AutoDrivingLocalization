...
class DWM1001_API_COMMANDS:
        ...
        def set_position(self, arrayData, i):
                x = float(arrayData[4+6*i])
                y = float(arrayData[5+6*i])
                z = float(arrayData[6+6*i])
                return [x, y, z]
        
        def set_node_id(self, arrayData, i):
                node_id = arrayData[3+6*i]
                
                return node_id
                
        def set_node_number(self, arrayData):
                number_of_anchors = int((len(arrayData) - 7)/6)
                
                return number_of_anchors
                
        def set_arrayData(self, serialData):
                serialData = [x.strip() for x in serialData.strip().split(',')]
                
                return serialData
                
        def set_range(self, arrayData, i):
                range = float(arrayData[7+6*i])