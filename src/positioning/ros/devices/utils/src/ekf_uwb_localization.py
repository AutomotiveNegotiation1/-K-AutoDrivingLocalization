...
class UWBLocalization:
    ...
    def __init__(self, AN_infos=None):
        ...
        self.AN_subscribers[AN_info] = rospy.Subscriber('/ublox/imu/twist', imu_message, self.add_imu)
        
        # tag subscriber def
        self.kalman_filter_tag = FusionEKF()
    ...
    def create_tag_subscriber_func(self, name):
        ...
        def add_ranging(msg):
            if msg.anchorId in self.anchor_poses:
                ...
                print()
                # for dev in self.TA_list.TagID:
                self.kalman_filter_tag.process_uwb_data({'anchor_position': anchor_pose, 'distance': anchor_distance})  # Update the FusionEKF with UWB data
                tag_x = self.kalman_filter_tag.get_state()[0]
                tag_y = self.kalman_filter_tag.get_state()[1]
                self.live_plotter.add_data_point('tag', name, tag_x, tag_y)
                # time.sleep(1)
                # current_time = datetime.datetime.now()
                # print(self.kalman_filter_tag.get_covariance()[:, 0])
                # self.live_plotter.add_data_P_matrix("P_matrix", current_time.strftime('%S'), self.kalman_filter_tag.get_covariance())
                    
        return add_ranging
    
    def add_imu(self, msg):
        dt = msg.dt
        quaternion = np.array([msg.quaternion_x, msg.quaternion_y, msg.quaternion_z, msg.quaternion_w])
        angular_velocity = np.array([msg.angular_x, msg.angular_y, msg.angular_z])
        acceleration = np.array([msg.linear_x, msg.linear_y, msg.linear_z])
        imu_data = {'dt': dt, 'acceleration': acceleration, 'angular_velocity': angular_velocity, 'orientation': quaternion}
        self.kalman_filter_tag.process_imu_data(imu_data)  # Update the FusionEKF with IMU data
