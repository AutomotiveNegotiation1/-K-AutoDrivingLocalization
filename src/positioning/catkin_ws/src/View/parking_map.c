...
int main(int argc, char *argv[])
{
    ...
    pixbuf = gdk_pixbuf_new_from_file_at_scale("yellow_car.png", car_width, car_height, TRUE, NULL);
    pixbuf = gdk_pixbuf_rotate_simple(pixbuf, GDK_PIXBUF_ROTATE_COUNTERCLOCKWISE);
    gImage = gtk_image_new_from_pixbuf(pixbuf);
    gtk_fixed_put(gFixed, gImage, 100 - center_offset_y, 98 - center_offset_x);
...
gpointer cctv_thread_run(gpointer data)
{
    ...
    while (1)
    {
        ...
        {
            gtk_widget_show(gImage);
            gtk_fixed_move(gFixed, gImage, cctv_x - center_offset_y, 80 - center_offset_x);
        }
...
gboolean manually_draw()
{
    ...
    // printf("cur_paint : %d, currnet_mstime : %f, cctv_mstime : %f\n", cur_paint, current_mstime, cctv_mstime);
...
void set_mode(double _uwb_x, double _uwb_y, double _cctv_x, double _cctv_y)
{
    ...
    ************************************************/
    ...
    // if(!isUpdate_uwb && isUpdate_cctv) {
    //     cur_paint = FROM_CCTV;
    // } 
    // else if (_uwb_y < 130.0 && _uwb_x > 218.0 && _uwb_x < 440.0)
    // {
    //     cur_paint = FROM_CCTV;
    // }
    // else
