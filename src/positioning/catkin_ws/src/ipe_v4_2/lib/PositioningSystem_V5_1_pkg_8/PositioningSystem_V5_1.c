  ...
  if (PositionVector_data[1] == 5.0) {
    ...
    if (PositionVector_data[2] != 0.0) {
      ...
      EKF_UWB_SLAM_IMU_1(prev_pos, &FiFoIMUgyro[9], PositionVector_data[0],
                         ...
                         FiFoSLAMUWBtime[99], &Posn_tmp, &SLAMpos_idx_1,
                         &SLAMpos_idx_2);
      ...
      if (PositionVector_data[2] != 0.0) {
        if (firstV == 0.0) {
          ...
          if (FiFoSLAMtime[98] < FiFoUWBtime[99] + 0.05) {
             ...
             * tempEulr); */
            /*  if Front == 1 */
            Posn_tmp.re = FiFoUWBhead[99] * 0.0;
            Posn_tmp.im = FiFoUWBhead[99];
            b_exp(&Posn_tmp);
            for (i = 0; i < 3; i++) {
            ...
            dc.re = FiFoUWBpos[99].re +
                    (CAMPos.re * Posn_tmp.re - CAMPos.im * Posn_tmp.im);
            dc.im = FiFoUWBpos[99].im +
                    (CAMPos.re * Posn_tmp.im + CAMPos.im * Posn_tmp.re);
             ...
             * yaw])); */
            /*  Eulr_r = quaternionToEulerYXZ(QuatInit, eulr2dcm([rollT  pitchT
             * yawT])); */
            /*  x(1) = PosSLAMc(1); */
            /*  x(2) = PosSLAMc(2); */
            /*  x(3) = PosSLAMc(3); */
          ...
          if (SLAMSet == 0.0) {
            if ((EKFpos.re != 0.0) || (EKFpos.im != 0.0)) {
              ...
              Posn_tmp.im = FiFoUWBhead[99];
              b_exp(&Posn_tmp);
              SLAMpos_idx_0 = CAMPos.re * Posn_tmp.re - CAMPos.im * Posn_tmp.im;
              SLAMpos_idx_1 = CAMPos.re * Posn_tmp.im + CAMPos.im * Posn_tmp.re;
              if ((rt_hypotd_snf(
                ...
                /*  PosHF = EKFpos; */
                /*  HeadingHF = EKFhead(2); */
                Posn_tmp.re = prev_pos[1] * 0.0;
                Posn_tmp.im = -prev_pos[1];
                b_exp(&Posn_tmp);
                PrevPosHF.re = EKFpos.re - (CAMPos.re * Posn_tmp.re -
                                            CAMPos.im * Posn_tmp.im);
                PrevPosHF.im = EKFpos.im - (CAMPos.re * Posn_tmp.im +
                                            CAMPos.im * Posn_tmp.re);
            ...
            b_exp(&Posn_tmp);
            PrevPosHF.re =
                EKFpos.re - (CAMPos.re * Posn_tmp.re - CAMPos.im * Posn_tmp.im);
            PrevPosHF.im =
                EKFpos.im - (CAMPos.re * Posn_tmp.im + CAMPos.im * Posn_tmp.re);
            HeadingHF = prev_pos[1];
            for (i = 0; i < 99; i++) {
              FiFoSLAMUWBpos[i] = FiFoSLAMUWBpos[i + 1];
              FiFoSLAMUWBhead[i] = FiFoSLAMUWBhead[i + 1];
