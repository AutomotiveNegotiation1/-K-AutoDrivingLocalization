clear all;
% fname = 'rosbag\test_0818_v4.mat';
fname = 'rosbag/test_b1_line_20220803_v1.mat';
...
for i = 1 : Sens.length
    ...
    if ((i>50)&&(UWB.flag))  % INS/UWB
        ...
        if UWB.num >= 3
            [UWB] = uwb_LS_positioning_3(UWB, Acc_Pos(:,i));
    ...
    if ((i>50) && (LiDAR.flag) && ~any(isnan(Acc_Pos(:,i))))
        
        MapDownsampled = pcdownsample(pcdenoise(pcdMap),'gridAverage',0.1);
        [indices, ~] = findNeighborsInRadius(MapDownsampled, Acc_Pos(:,i)', LiDAR.map_radius_max);
        ...
        moving = pctransform(LiDAR.ptSub, rigid3d(Cb2n, Acc_Pos(:,i)'));
        ...
        pos_new = Acc_Pos(:,i)'*tform.Rotation + tform.Translation;
...
figure(78);hold off;plot(Acc_Pos(1,:),Acc_Pos(2,:));axis equal;hold on;plot(UWB_LS_Pos(1,:),UWB_LS_Pos(2,:),'ro');axis equal;axis([-2+0.9 2+0.9 -1 28])
