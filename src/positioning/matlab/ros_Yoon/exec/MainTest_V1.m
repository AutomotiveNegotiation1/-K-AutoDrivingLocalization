...
%     az_c(k) = -gravity_t*cos(theta_org(k))*cos(phi_org(k));
     ...
     z = EulerToQuaternion(phi_org(k), theta_org(k), eurl_gyro(3,k));
    [phi(k), theta(k), psi(k)] = EulerKalman(A, z);
    att_kf = EulerToQuaternion(phi(k), theta(k), psi(k));
    C_b2n = C_b2n_next;
    C_n2b = C_n2b_next;
...
%     a_e(:,k) = C_b2n*[ax ay az]';
    ...
    temp_b(:,k) = gravity*[sin(theta(k));-cos(theta(k))*sin(phi(k));-cos(theta(k))*cos(phi(k))];
    temp_c(:,k) = [0 vz_b -vy_b; -vz_b 0 vx_b;vy_b -vx_b 0]*[p;q;r];
