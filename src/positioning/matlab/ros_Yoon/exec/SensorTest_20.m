...
for i = 1 : Sens.length
    ...
    Ori(i) = FUSE(IMUacc(i,:)-b_acc_o,IMUgyro(i,:)-b_gyro);
    ML = eulerd(Ori(i),'XYZ','frame')/180*pi;
...
%         used_psi = ML(3)+pi/2;
%         used_phi_next = ML(1);
%         used_theta_next = ML(2);
%         used_psi_next = ML(3)+pi/2;
        used_phi = gyro_phi;
...
%         x = [Acc_Pos(:,i-1);Acc_Vel(:,i-1);acc_b(:,1)-Acc_Steady(:,i)-Acc_delEuler(:,i);b_acc_o'];
        TEMP = rotationVectorToMatrix([0;0;0])'*[acc_b(:,i)-Acc_PQR(:,i)-Acc_Steady(:,i)-Acc_delEuler(:,i)];
        TEMP_bias = rotationVectorToMatrix([0;0;0])'*b_acc_o';
%         TEMP = rotationVectorToMatrix([0;0;gyro_psi])'*[acc_b(:,i)-Acc_PQR(:,i)-Acc_Steady(:,i)-Acc_delEuler(:,i)];
%         TEMP_bias = rotationVectorToMatrix([0;0;gyro_psi])'*b_acc_o';  
%         x = [Acc_Pos(1:2,i-1);0;Acc_Vel(1:2,i-1);0;TEMP(1:2,1);0;TEMP_bias];
...
%         z = [UWB_LS_Pos(:,k);UWB_LS_Vel(:,k)];
        ...
        [acc_b_phi,acc_b_theta] = EulerAcc(acc_b(1,i)-acc_g(1)-Acc_PQR(1,i), acc_b(2,i)-acc_g(2)-Acc_PQR(2,i), acc_b(3,i)-acc_g(3)-Acc_PQR(3,i));
        acc_b_phi_filtered = filter(LPFG1,acc_b_phi);
        acc_b_theta_filtered = filter(LPFG2,acc_b_theta);
