...
%         fprintf("\t\t\t\t\tUWB : %f\n", UWB.time);
    ...
    if Nav.mode == 0  % Alignment
        [Nav] = ins_alignment(Nav, IMU_O(NumDelay), i, NaviMode);
...
%         end
        ...
        [Nav] = navi_propagation_15state_2(Nav, IMU_O(NumDelay),i);
%         [Nav] = navi_propagation_15state_3(Nav, IMU_O(NumDelay), i);
%         [Nav] = navi_propagation_15state_5(Nav, IMU_O(NumDelay), i);
...
%             [Nav, Ani] = navi_uwb_update(Nav, UWB, Ani);
            [Nav, Ani] = navi_uwb_update_2(Nav, UWB, Ani);
            if DrawPerPlot == 1
                   figure(100);hold on;plot(UWB_LS_Pos(1), UWB_LS_Pos(2), '*r','LineWidth',1.3);  grid on; axis equal; hold on;
            end
            UWB.flag = false;
