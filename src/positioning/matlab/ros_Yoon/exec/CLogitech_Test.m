...
%     image_left = rgb2gray(img(:, 1 : width/2, :));
    image_left = rgb2gray(img);
%         image_right = rgb2gray(img(:, width/2 +1: width, :));
figure(101);hold off;imshow(image_left);
...
%             cameraLocation = cameraPose.Translation;
            cameraLocation = rotationMatrixx*translationVectorx';
            Pos(po,:) = RefPosTable(CB_ID1,:)-[cameraLocation(1) cameraLocation(3)]
