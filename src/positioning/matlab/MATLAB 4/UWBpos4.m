function [UWBout] = UWBpos4(Ln, Lp, LnC, TagNum, Nanchor, RxIDin, RxDistin, s_time, tag_pos_b, xain, yain, prevTagPos, prevTagHeading)
%[tag_pos_est, heading_est,tag_pos_est_aver,headingest_a_aver_v]
average_len = 10;
NumInterpPoint = 16;
tag_center_vel_est = 0;

persistent r InterpPosition Tag_Pos_List TagDistInitCount TagDistInit heading_est_a centerest_a centerest_a_aver headingest_a_aver RxIDprev PPprev RxIDprevLen DistList DistListTime PrevPos PrevHeading
...
% DelRxDist = [];
% for dfd = 1 : length(RxDist)
%     if (mean(abs(prevDist(RxID(dfd),TagNum) - RxDist(dfd))) > 1) & (r > 50) %sum(mean(prevDist,2)~=prevDist(:,1))
%         RxDistin(dfd) = prevDist(RxID(dfd),TagNum);
% %         DelRxDist = [DelRxDist dfd];
%     end
% end
% if ~isempty(DelRxDist)
%     RxDist(DelRxDist) = [];
%     RxID(DelRxDist) = [];
%     Nanchor = Nanchor - length(DelRxDist);
% end
...
% figure(100);hold on;plot(s_time*ones(1,length(DistPredicted(:,1))),DistPredicted(:,1),'b.');
% figure(101);hold on;plot(s_time*ones(1,length(DistPredicted(:,2))),DistPredicted(:,2),'g.');
% figure(102);hold on;plot(s_time*ones(1,length(DistPredicted(:,3))),DistPredicted(:,3),'r.');
% figure(103);hold on;plot(s_time*ones(1,length(DistPredicted(:,4))),DistPredicted(:,4),'k.');
...
%     end
    ...
    tag_pos_est_t2 = PrevPos;
    heading_est_t2 = PrevHeading;
...
elseif r == 20*Lp
    ...
    for PPC = 1 : Lp
        [InterpPosition(PPC,1)] = InterpPos2(Tag_Pos_List(:,1,PPC),real(Tag_Pos_List(:,2,PPC)),s_time,1);
        [InterpPosition(PPC,2)] = InterpPos2(Tag_Pos_List(:,1,PPC),imag(Tag_Pos_List(:,2,PPC)),s_time,1);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     tag_pos_est_aver = get_tag_pos(centerest_a_aver(end,1)+j*centerest_a_aver(end,2), headingest_a_aver(end), tag_pos_b);
    ...
    tag_pos_est_aver = tag_pos_b*exp(j*(mod(headingest_a_aver(end),2*pi)))+Xt_c_e+j*Yt_c_e;
