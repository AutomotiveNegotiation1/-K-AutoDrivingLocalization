...
aa = rosbag('20230811_driving_fast.bag');
...
%         ya = yain;
        ...
        if (Nanchor > 1) && (isempty(find(RxDistUWB(1:Nanchor)==0, 1)))
            [UWBout] = UWBpos7(Ln, Lp, LnC, PP, Nanchor, RxIDUWB, RxDistUWB, s_time, tag_pos_b, xain, yain, TagPos, -kf_psi);
            %             UWBoutTot(k,:) = UWBout;
...
%             end
            ...
            if ((k0(5)>50))  % INS/UWB
                ...
                if (kalman_on == 1) && (init_flag > 2)  && (UWBErrSum(k) <0.2)%&& (Nanchor>2) && (Reliability >2)
                ...
                if (kalman_on == 1) && (init_flag > 2) && (UWBErrSum(k) <0.2)
                    R = 1e-1*eye(6);
