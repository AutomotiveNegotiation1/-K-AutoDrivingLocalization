function [UWBout] = UWBpos6(Ln, Lp, LnC, TagNum, Nanchor, RxIDin, RxDistin, s_time, tag_pos_b, xain, yain, prevTagPos, prevTagHeading)
...
prevDistTemp1 = transpose(anch_pos);
prevDistTemp2 = reshape(prevTagPos,1,4);
prevDistTemp3 = transpose(anch_pos) - reshape(prevTagPos,1,4);
prevDist = abs(prevDistTemp3);
...
%% DistList/DistListTime matrix mapping
for q = 1 : Nanchor
...
%% DistPredicted matrix mapping
DistPredicted = zeros(Ln,Lp);

for q = 1 : Ln
    for p = 1 : Lp
        for rq = 1 : average_len
            if (s_time-DistListTime(q,p,rq)) > 2
                DistList(q,p,rq) = 0;
                DistListTime(q,p,rq) = 0;
            end
        end
        if (s_time - DistListTime(q,p,end))< 0.3
...
if TagNum == 4
    ...
    if min(sum(DistPredicted)) > 0
        xyTemp = xa+j*ya;
        [tag_pos_est_t3, heading_est_t3, ErrSum] =  GetInitPos3(xa,ya, DistPredicted(1:LnC,:),xyTemp,tag_pos_b,LnC,4,prevTagPos,prevTagHeading);
    else
