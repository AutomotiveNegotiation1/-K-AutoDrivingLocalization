function [out_flag_o, tag_pos_est2, heading_est2] = UWBposEnh(Ln, Lp, PP, Nanchor, RxID, RxDist, s_time, tag_pos_b, xa, ya, tag_pos_est_2, heading_est_2)
            
persistent RxDistMat s_time_list RxIDList NanchorList PPlist tag_pos_est_list heading_est_list l tag_pos_est heading_est
...
% if ((s_time - s_time_list(3))>0.02)
%     RxDistMat = zeros(Ln,Lp);
%     RxIDList = zeros(Ln,Lp);
%     NanchorList = zeros(1,4);
%     PPlist = zeros(1,4);
    tag_pos_est2 = tag_pos_est;
    heading_est2 = heading_est;
    out_flag_o = 0;
%     tag_pos_est_list = zeros(Lp,Lp);
%     heading_est_list = zeros(1,Lp);
% else
%     out_flag_o = 0;
%     tag_pos_est2 = zeros(1,4);
%     heading_est2 = 0;
% end
    if PP == 1
        RxDistMat = zeros(Ln,Lp);
...
%     PPlist(l) = PP;
    ...
    if (length(find(RxDistMat(:)>0)) >= 8) && (PP == 4)
            out_flag_o = 1;

        [tag_pos_est2, heading_est2] = GetInitPos(xa,ya,RxDistMat,anch_pos,tag_pos_b,Ln,Lp);
%         [tag_pos_est2, heading_est2] = GetUWBPos_v2(xa, ya, RxDistMat, tag_pos_b);
%         [tag_pos_est2, heading_est2] = GetUWBPos_v3(xa, ya, RxDistMat, tag_pos_b,tag_pos_est_2, heading_est_2);
    end
