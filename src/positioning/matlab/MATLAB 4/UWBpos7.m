function [UWBout] = UWBpos7(Ln, Lp, LnC, TagNum, Nanchor, RxIDin, RxDistin, s_time, tag_pos_b, xain, yain, prevTagPos, prevTagHeading)
...
for q = 1 : Ln
    for p = 1 : Lp
        for rq = 1 : average_len
            if (s_time-DistListTime(q,p,rq)) > 0.3
                DistList(q,p,rq) = 0;
                DistListTime(q,p,rq) = 0;
            end
        end
%         if length(find(DistList(q,p,:)~=0))>3
%             DistPredicted(q,p) = InterpPos3(DistListTime(q,p,:),DistList(q,p,:),s_time,2);
