...
aa = rosbag('20230811_driving_slow.bag');
...
%         ya = yain;
        ...
        if Nanchor > 1
            [UWBout] = UWBpos3(Ln, Lp, LnC, PP, Nanchor, RxIDUWB, RxDistUWB, s_time, tag_pos_b, xain, yain);
            %             UWBoutTot(k,:) = UWBout;
            ...
            if abs(TagRoute)>2
                heading_est = angle(TagRoute*exp(j*pi/2));
            else
                heading_est = angle(mean(mean(TagPosBuff([2 4],:)-TagPosBuff([1 3],:))));
            end
            [out_flag, tag_pos_est2, heading_est2] = UWBposEnh(LnC, Lp, PP, Nanchor, RxID, RxDist, s_time, tag_pos_b, xain, yain, tag_pos_est, heading_est);
            
            tag_pos_est_eh(:,k) = zeros(4,1);
            if out_flag == 1
                tag_pos_est_eh(:,k) = tag_pos_est2;
%                 tag_pos_est_t = tag_pos_est;
                heading_est_eh(k) = heading_est2;
%                 heading_est_t = heading_est;
                
            else
%                 tag_pos_est = tag_pos_est_t;
%                 heading_est = heading_est_t;
                tag_pos_est_eh(:,k) = 0;
%                 tag_pos_est_t = tag_pos_est;
                heading_est_eh(k) = 0;
            end
            ...
            if ((k0(5)>50))  % INS/UWB
                ...
                if (kalman_on == 1) && (init_flag > 2)%&& (Nanchor>2) && (Reliability >2)
...
%             figure(1);hold off;plot(cent_pos_est(1),cent_pos_est(2),'.');hold on;plot(xa,ya,'o');plot(real(TagPos(1:2)),imag(TagPos(1:2)),'ro');plot(real(TagPos(3:4)),imag(TagPos(3:4)),'ko');axis([-5 30 -5 20])
%             if min_stamp_i < 5
%     
...
figure(2003);plot(TagPosTot,'.')
figure(2004);plot(TagUWBPosTot,'.')
