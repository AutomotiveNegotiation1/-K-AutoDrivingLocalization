            ...
            for l4 = 1 : max(1,Pos2CL(4))
                ...
                for oer = 1 : length(NZeroId)
                    ...
                    for ork = oer + 1 : length(NZeroId)
                        ...
                        Cand_absH = Cand_absH + abs(abs((lP(PPI)-lP(PPQ))/(tag_pos_b(PPI)-tag_pos_b(PPQ)))-1)^2;
                    end
                end
                Cand_Err = Cand_absH/cnt;
                Cand_HErr = abs(abs(Cand_H/cnt)-1)^2;

                if ((Cand_HErr + Cand_Err) < (Sel_HErr + Sel_Err)) && (cnt~=0)
                    Sel_H = Cand_H/cnt;
                    Sel_HErr = Cand_HErr;
                    Sel_Err = Cand_Err;
