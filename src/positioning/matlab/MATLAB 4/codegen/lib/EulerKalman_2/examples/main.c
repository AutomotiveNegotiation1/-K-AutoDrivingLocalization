...
#include "EulerKalman_2_terminate.h"
...
void main_EulerKalman_2(void)
{
  ...
  b_x_tmp[2] = x_tmp[2];
  b_x_tmp[3] = x_tmp[3];
  EulerKalman_2(b_x_tmp, dv, x_tmp, &phi, &theta, &psi);
}
 ...
 * Return Type  : void
 */
void main_EulerToQuaternion(void)
{
  double z[4];
  double phi_tmp;
  /* Initialize function 'EulerToQuaternion' input arguments. */
  phi_tmp = argInit_real_T();
  /* Call the entry-point 'EulerToQuaternion'. */
  EulerToQuaternion(phi_tmp, phi_tmp, phi_tmp, z);
}
