...
%         uwb.time = cellfun(@(m) double(m.Header.Stamp(1).Sec),msgUwb) + cellfun(@(m) double(m.Header.Stamp(1).Nsec),msgUwb)/10^9;
        ...
        UWB.anchor(:,4) = -2;
        UWB.anchor(1,2:4) = [ 1.8 7.2 2.18]; % 991B [Y, X]
        UWB.anchor(2,2:3) = [nan nan]; % 4630
        UWB.anchor(3,2:3) = [nan nan]; % 4302
        UWB.anchor(4,2:4) = [-1.8 7.2 2.18]; % 1121
        UWB.anchor(5,2:4) = [-1.8 0   2.18]; % 439D
        UWB.anchor(6,2:3) = [nan nan]; % 89A4
        UWB.anchor(7,2:3) = [nan nan]; % 4599
        UWB.anchor(8,2:4) = [ 1.8 0   1.69]; % 9B8F
...
%     drawnow
%         addpoints(h,UWB_M_Pos(1,k), UWB_M_Pos(2,k), UWB_M_Pos(3,k));
        addpoints(h,Acc_Pos(1,i), Acc_Pos(2,i), Acc_Pos(3,i));
        xline(0.5, 'r:');
        xline(-0.5, 'r:');
