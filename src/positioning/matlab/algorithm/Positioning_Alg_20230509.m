...
for p = 1 : 4
    for n = 1 : 4
        for m = 1 : 4
            if n~=m
%                 if (n==1 && (p == 1 || p ==2) )|| (m==1  && (p == 1 || p ==2)) ||(n==3  && (p == 1 || p ==2))|| (m==3  && (p == 1 || p ==2))
                if (n>100)    
                else
                B(Lj,1) = -2*(xa(n)-xa(m));
                B(Lj,2) = -2*(ya(n)-ya(m));
...
centerpos_error = sqrt((Xt_c_e-tag_center(1))^2+(Yt_c_e-tag_center(2))^2)
heading_error = abs(heading-heading_est)*180/pi
