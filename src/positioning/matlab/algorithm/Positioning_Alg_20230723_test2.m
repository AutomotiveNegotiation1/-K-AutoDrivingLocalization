...
%         dist(2,1:Ln) = dist_a(2,1:Ln)+3;
    ...
    elseif r == 10*Lp
        ...
        s_time_prev = 0;
        for LLp= 1 : Lp
            for lo = NumInterpPoint : -1 : 1
                Tag_Pos_List(lo,:,LLp) = [s_time(r-lo+1) tag_pos_est(LLp)];
            end
        end
        ...
        for PP = mod(r-1,Lp)+1:mod(r-1,Lp)+1
            ...
            if Ln >= 4
                for NN = 1 : Ln
                    for MM = NN+1 : Ln
                        for LL = MM+1 : Ln
                            [Pos(:,UU),Prob(UU)] = ThreeAnchPos3([xa(NN) xa(MM) xa(LL)],[ya(NN) ya(MM) ya(LL)],[dist(NN,PP) dist(MM,PP), dist(LL,PP)]);
                            UU = UU + 1;
                        end
                    end
                end
                %             Xt_pred_c = InterpPosition(PP,1) + j*InterpPosition(PP,2);
                [v,ind_m] = min(Prob);
                Xt_pred_c = Pos(1,ind_m)+j*Pos(2,ind_m);
                InterpPosition(PP,1) = real(Xt_pred_c);
                InterpPosition(PP,2) = imag(Xt_pred_c);
            else
                Xt_pred_c = InterpPosition(PP,1) + j*InterpPosition(PP,2);
            end
            ...
            for NN = 1 : Ln
                for MM = NN+1 : Ln
                    if NN~=MM
                        %if NN == mod(r-1,Lp)
                        [Pos2,Prob2]=TwoAnchPos3([xa(NN) xa(MM)], [ya(NN) ya(MM)], [dist(NN,PP) dist(MM,PP)],[real(Xt_pred_c) imag(Xt_pred_c)],[Xt_c_e Yt_c_e],anch_pos(1:Ln),dist(:,PP));
                        ...
                        if (abs(Pos2C_1-Xt_pred_c)) < (abs(Pos2C_2-Xt_pred_c))
                            PosC = [PosC Pos2C_1];
                            PosC_E = [PosC_E abs(abs(Pos2C_1-Xt_pred_c))^2];
                        else
                            PosC = [PosC Pos2C_2];
                            PosC_E = [PosC_E abs(abs(Pos2C_2-Xt_pred_c))^2];
                        end
            ...
            InterpPosition(PP,1) = real(PosC_Est);
            InterpPosition(PP,2) = imag(PosC_Est);

%             end

            Xt_Center = mean(InterpPosition(:,1))+j*mean(InterpPosition(:,2));
            head_me = 0;
            for ku = 2 : Lp
                head_me = head_me + ((InterpPosition(ku,1)+j*InterpPosition(ku,2))-(InterpPosition(ku-1,1)+j*InterpPosition(ku-1,2)))/(xtc_b(ku)-xtc_b(ku-1));
            end

            Tag_Pos_List(1:2,:,PP) = Tag_Pos_List(2:3,:,PP);
            Tag_Pos_List(3,:,PP) = [s_time(r) InterpPosition(PP,1)+j*InterpPosition(PP,2)];
            Xt_c_e = mean(InterpPosition(:,1));
            Yt_c_e = mean(InterpPosition(:,2));
        end
        ...
        tag_center_pos_est = Xt_Center;
        tag_pos_est = transpose(InterpPosition(:,1))+j*transpose(InterpPosition(:,2));
