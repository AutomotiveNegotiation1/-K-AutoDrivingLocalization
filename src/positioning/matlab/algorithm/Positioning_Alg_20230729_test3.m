...
%         dist(2,1:Ln) = dist_a(2,1:Ln)+3;
    ...
    Ln = mod(floor(4*rand(1)),4)+1;

    ReID = randperm(length(xa));
    RxID = ReID(1:Ln);
    RxTag = mod(r-1,Lp)+1;
    PP = RxTag;

    RxDist = dist(RxID(1:Ln),PP);
...
%             for PP = mod(r-1,Lp)+1:mod(r-1,Lp)+1
                TagDistInit(NN,PP) = TagDistInit(NN,PP)*(div-1)/div+RxDist(NNid)/div;
%             end

        end
        latest_dist_1 = latest_dist_0;
        latest_dist_0 = RxDist;
        latest_dist_1_anch = latest_dist_0_anch;
        latest_dist_0_anch = RxID;
        latest_dist_1_tag = latest_dist_0_tag;
        latest_dist_0_tag = PP;
...
%             end
        ...
        [tag_pos_est, heading_est] =  GetInitPos(xa,ya,TagDistInit(,anch_pos,tag_pos_b,Ln,Lp);
        ...
        for PP = mod(r-1,Lp)+1:mod(r-1,Lp)+1
            [tag_pos_est, heading_est, cand_tag_pos] = GetPos(xa,ya,dist(:,PP),anch_pos,tag_pos_b,Ln,PP,InterpPosition(:,1)+j*InterpPosition(:,2));

            [tag_pos_est, heading_est, cand_tag_pos] = GetPosRefine(xa,ya,dist(:,PP),anch_pos,tag_pos_b,Ln,PP,tag_pos_est, heading_est, cand_tag_pos);

            Tag_Pos_List(1:NumInterpPoint-1,:,PP) = Tag_Pos_List(2:NumInterpPoint,:,PP);
            Tag_Pos_List(NumInterpPoint,:,PP) = [s_time(r) cand_tag_pos];
