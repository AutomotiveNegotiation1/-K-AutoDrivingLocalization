...
while((length(UWBMsg{1})>=k0(1))&&(length(UWBMsg{2})>=k0(2))&&(length(UWBMsg{3})>=k0(3))&&(length(UWBMsg{4})>=k0(4))&&(length(IMUMsg)>=k0(5)))
    ...
    if min_stamp_i == 5
        ...
        Xbar = PredEKF_Simple(Xhat,acc,GyroD,0.03);
        PosH(qv) = Xbar(1)+Xbar(2)*j;
        Heading(qv) = Xbar(7);
    ...
    elseif min_stamp_i<5
        ...
        % if (Nanchor > 2) && (isempty(find(RxDistUWB(1:Nanchor)==0, 1)))
            % UWBpos_Base(Ln, Lp, LnC, PP, Nanchor, RxIDUWB, RxDistUWB, s_time, tag_pos_b, xain, yain, TagPos, -kf_psi);
            % [Pos1, Pos2] = UWBpos_V2_2(Ln, Lp, LnC, PP, Nanchor, RxIDUWB, RxDistUWB, s_time, tag_pos_b, xain, yain, TagPos, -kf_psi);
            [Pos1, Pos2] = UWBpos_V2_2(Nanchor, RxIDUWB, RxDistUWB, xain, yain);
            
            
            if dt_uwb <0.01
                alpe = angle((Pos1-Prev_Pos)*conj(tag_pos_b(min_stamp_i)-tag_pos_b(prev_tag_num)));
                if qv == 1
                    alpe_t1 = alpe;
                else
                    alpe_t = Heading_Prev-mod(-alpe,2*pi);
                    alpe_t_r = sign(alpe_t)*floor(abs(alpe_t)/(2*pi));
                    alpe_t1 = -alpe+alpe_t_r*2*pi;
                    if alpe_t1 - Heading_Prev > pi
                        alpe_t1 = alpe_t1 - 2*pi;
                    elseif alpe_t1 - Heading_Prev < -pi
                        alpe_t1 = alpe_t1 + 2*pi;
                    end
                end
            end
            ...
            if (k > 10)
                GyroD = mean(IMUgyro_c_fifo)*pi/180;
                acc = mean(IMUacc_c_fifo);
                Xbar = PredEKF_Simple(Xhat,acc,GyroD,dt_uwb);
                A = makePredA_Simple(Xhat,acc,GyroD,dt_uwb);
                [Zv,H] = EstEKF_Simple(Xbar,acc,GyroD,dt_uwb,[xt_b(min_stamp_i) yt_b(min_stamp_i)]);
                Q = eye(8)*1e-7;
                ...
                % Pos1 = findMatPos([Pos1,Xbar(1:2))
                TT = [Pos1 Pos2];
                [PosDistV,PosDistI] = min(abs((Xbar(1)+j*Xbar(2))-(TT-(xt_b(min_stamp_i)+j*yt_b(min_stamp_i))*exp(-j*Xbar(7)))));
                Pos1 = TT(PosDistI);
                
                if TT(1) ~= 0
                    VarPos = mean(abs(TT-Pos1));
                ...
                Xhat = transpose(Xbar) + K*(transpose([alpe_t1 real(Pos1) imag(Pos1)])-transpose(Zv));
                P = P - K*H*P;

                PosH(qv) = Xhat(1)+j*Xhat(2);
                Heading(qv) = Xhat(7);
                alpe_a(qv) = alpe_t1;
                PosC(qv,min_stamp_i) = Pos1;
                figure(20);hold on;plot(Xhat(1)+j*Xhat(2),'r.');
                % figure(20);hold on;plot(Pos1,'b.');
            else
                Heading(qv) = 0;
            end
            Heading_Prev = Xhat(7);
            % if min_stamp_i==4
            %     figure(12);hold on;plot(Pos1,'.');
            % end
            % for kl = 1 : length(Pos2)
            %     figure(12);plot(Pos2(kl),'.')
            % end
            % Heading(qv) = alpe_t1;
            k0(PP) = k0(PP) + 1;
            qv = qv + 1;
        ...
        %         Xbar = PredEKF_Simple(Xhat,acc,GyroD,dt_uwb);
        %         A = makePredA_Simple(Xhat,acc,GyroD,dt_uwb);
        %         [Zv,H] = EstEKF_Single_Simple(Xbar,acc,GyroD,dt_uwb,[xt_b(min_stamp_i) yt_b(min_stamp_i)], xain(RxID), yain(RxID));
        %         Q = eye(8)*1e-7;
        %         Q(1:4,1:4) = eye(4)*1e-5;
        %         R = eye(1)*1e-2;
        %         temp(qv,:) = GyroD;
        ...
        %         % Pos1 = findMatPos([Pos1,Xbar(1:2))
        % 
        %         Xhat = transpose(Xbar) + K*(transpose([RxDist^2])-transpose(Zv));
        %         P = P - K*H*P;
        % 
        %         PosH(qv) = Xhat(1)+j*Xhat(2);
        %         Heading(qv) = Xhat(7);
