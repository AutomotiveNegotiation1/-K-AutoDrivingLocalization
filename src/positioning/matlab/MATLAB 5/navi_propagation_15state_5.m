function [Nav] = navi_propagation_15state_5( Nav, IMU, i  )

    persistent qua_gyro
    persistent firstRun
    
    if isempty(firstRun)|| (sum(Nav.output.att == [1 0 0 0]')==4)
        qua_gyro = [1 0 0 0]';
        firstRun = 0;
    end
    dt = IMU.dt;
    ...
    T_ang = w_raw-b_gyro;
    p = T_ang(1); q=T_ang(2); r=T_ang(3);
    
    AA = eye(4)+dt*1/2*[0 -p -q -r;    p 0 r -q;    q -r 0 p;    r q -p 0    ];
    qua_gyro = AA*qua_gyro;
    eurl_gyro= dcm2eulr(qua2dcm(qua_gyro));
    
    [phi_org, theta_org] = EulerAcc(a_raw(1)-b_acel(1), a_raw(2)-b_acel(2), a_raw(3)-b_acel(3));

    z = EulerToQuaternion(phi_org, theta_org, eurl_gyro(3));
    [phi, theta, psi] = EulerKalman(AA, z);
    x_att = EulerToQuaternion(phi, theta, psi);
