...
while (length(IMUVMsg)>=k0(5))
    ...
    s_time_a(2) = double(IMUAMsg{k0(6)}.Header.Stamp.Sec)+double(IMUAMsg{k0(5)}.Header.Stamp.Nsec)/10^9;
    [min_stamp_v, min_stamp_i] = min(s_time_a);

    s_time = min_stamp_v;
    if min_stamp_v == 1
        dt = s_time - s_time_prev(1);
        s_time_prev(1) = s_time;
    else
        dt_acc = s_time - s_time_prev(2);
        s_time_prev(2) = s_time;
    end
...
%     s_time_prev = s_time;
    ...
    if (min_stamp_i == 1)
        ...
        IMUgyro(end,:)  = [IMUVMsg{k0(5)}.Vector.X IMUVMsg{k0(5)}.Vector.X IMUVMsg{k0(5)}.Vector.X];
    else
