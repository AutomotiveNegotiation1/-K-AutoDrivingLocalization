function [UWBout] = UWBpos_V2_1(Ln, Lp, LnC, TagNum, Nanchor, RxIDin, RxDistin, s_time, tag_pos_b, xain, yain, prevTagPos, prevTagHeading)
%[tag_pos_est, heading_est,tag_pos_est_aver,headingest_a_aver_v]
% Ln : 최대 앵커 수, Lp : 총 테그 수, LnC : 현재까지 사용된 앵커수,
% TagNum : RxDistin의 테그 번호, Nanchor : 테그로 거리정보가 들어온 앵커의 수

average_len = 30;
NumInterpPoint = 16;
tag_center_vel_est = 0;
tag_pos_est_t3 = zeros(1,4);
...
if Nanchor == 2
    [Pos] = TwoAnchPos5(xa(RxID), ya(RxID), RxDist);
    Pos1 = Pos(1,:)';
    Pos2 = Pos(2,:)';
    
elseif Nanchor >= 3
    [Pos] = ThreeAnchPos4(xa(RxID), ya(RxID), RxDist);
    Pos1 = Pos;
    Pos2 = Pos;
else 
...
for q = 1 : Ln
    for p = 1 : Lp
        ...
        for rq = 1 : average_len
            if (s_time-DistListTime(q,p,rq)) > 0.1
                DistList(q,p,rq) = 0;
...
%    if min(sum(DistPredicted)) > 0   
    if length(find(sum(DistPredicted) > 0))>3
        [tag_pos_est_t3, heading_est_t3, ErrSum] =  GetInitPos4(xa,ya, DistPredicted(1:LnC,:),xa+j*ya,tag_pos_b,LnC,4,prevTagPos,prevTagHeading);
    ...
    if sum(Tag_Pos_List(12,1,:)-Tag_Pos_List(1,1,:)) ~=0
    ...
    tag_pos_est_aver = tag_pos_b*exp(j*(mod(headingest_a_aver(end),2*pi)))+K_Xt_c_e+j*K_Yt_c_e;
    ...
    UWBout = zeros(1,21);
    UWBout(1:4) = real(PrevPos);
    UWBout(5:8) = imag(PrevPos);
    UWBout(9) = PrevHeading;
    UWBout(10:13) = real(PrevPos);
    UWBout(14:17) = imag(PrevPos);
    UWBout(18) = PrevHeading;
    UWBout(19:20) = [real(tag_center_vel_est) imag(tag_center_vel_est)];
    UWBout(21) = 100;
end

if TagNum == 4
    [heading_new] = GetHeading(xa,ya,DistPredicted,tag_pos_b);
    UWBout(22) = heading_new;
else
    UWBout(22) = 0;
end
