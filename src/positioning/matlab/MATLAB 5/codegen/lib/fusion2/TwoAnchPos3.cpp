                 ...
                 double Prob[2])
{
  ...
  for (int b_k{0}; b_k < 2; b_k++) {
    b_d_tmp_tmp = y_tmp[b_k + 2];
    d_tmp = y_tmp[b_k];
    b_y_tmp[b_k] = d_tmp * AA + b_d_tmp_tmp * b_d;
    b_y_tmp[b_k + 2] = d_tmp * -s + b_d_tmp_tmp * d_tmp_tmp;
  }
  ...
  for (int b_k{0}; b_k < 2; b_k++) {
    b_d_tmp_tmp = A[b_k + 2];
    d_tmp = A[b_k];
    X1[b_k] = (d_tmp * AA + b_d_tmp_tmp * -s) * d_idx_0 +
  ...
  for (int b_k{0}; b_k < 2; b_k++) {
    b_d_tmp_tmp = A[b_k + 2];
    d_tmp = A[b_k];
    X2[b_k] = (d_tmp * AA + b_d_tmp_tmp * -s) * d_idx_0 +
              (d_tmp * b_d + b_d_tmp_tmp * d_tmp_tmp) * d_idx_1_tmp;
  }
  AA = X1[0] + 0.0 * X1[1];
  s = X1[1];
  ...
  for (int b_k{0}; b_k < nx; b_k++) {
    tmp_data[b_k] =
        rt_hypotd_snf(anch_pos_data[b_k].re - AA, anch_pos_data[b_k].im - s);
  }
