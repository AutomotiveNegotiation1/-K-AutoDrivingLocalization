...
void sort(double x[16], int idx[16])
{
  ...
  if (ib > 0) {
    ...
    for (int b_k{0}; b_k < quartetOffset; b_k++) {
      i1 = perm[b_k] - 1;
      i3 = ((b_k - nNaNs) - ib) + 16;
      idx[i3] = idx4[i1];
      x[i3] = x4[i1];
  ...
  if (16 - nNaNs > 1) {
    ...
    while (i3 > 1) {
      ...
      for (int b_k{0}; b_k < i3; b_k++) {
        merge(idx, x, b_k * ib, quartetOffset, quartetOffset, iwork, xwork);
      }
      quartetOffset = ib;
    }
