                   ...
                   creal_T tag_pos_est[4], double &ErrSum)
{
  ...
  for (cnt = 0; cnt < 4; cnt++) {
    ...
    if (coder::sum(b_dist) != 0.0) {
      ...
      if (PPI > 2) {
        ...
        Cand_HeadingDiff = localSD->pd->PrevUWBpos[cnt].re;
        Cand_HErr = localSD->pd->PrevUWBpos[cnt].im;
        DistPrev[0] =
            rt_hypotd_snf(Pos3[0] - Cand_HeadingDiff, Pos3[1] - Cand_HErr);
        DistPrev[1] =
            rt_hypotd_snf(Pos2[0] - Cand_HeadingDiff, Pos2[2] - Cand_HErr);
        ...
        DistPrev[0] =
            rt_hypotd_snf(Pos2[0] - Cand_HeadingDiff, Pos2[2] - Cand_HErr);
        brm_tmp = localSD->pd->PrevUWBpos[cnt].re;
        b = localSD->pd->PrevUWBpos[cnt].im;
        DistPrev[1] =
            rt_hypotd_snf(Pos2[1] - Cand_HeadingDiff, Pos2[3] - Cand_HErr);
  ...
  localSD->pd->PrevUWBpos[0].re = Cand_HeadingDiff;
  localSD->pd->PrevUWBpos[0].im = Cand_HErr;
  Cand_HeadingDiff =
  ...
  localSD->pd->PrevUWBpos[1].re = Cand_HeadingDiff;
  localSD->pd->PrevUWBpos[1].im = Cand_HErr;
  Cand_HeadingDiff =
  ...
  localSD->pd->PrevUWBpos[2].re = Cand_HeadingDiff;
  localSD->pd->PrevUWBpos[2].im = Cand_HErr;
  Cand_HeadingDiff =
  ...
  localSD->pd->PrevUWBpos[3].re = Cand_HeadingDiff;
  localSD->pd->PrevUWBpos[3].im = Cand_HErr;
  return heading_est;
}
...
void GetInitPos4_init(UWBPos6 *aInstancePtr)
{
  UWBpos6StackData *localSD;
  localSD = aInstancePtr->getStackData();
  std::memset(&localSD->pd->PrevUWBpos[0], 0, 4U * sizeof(creal_T));
}
