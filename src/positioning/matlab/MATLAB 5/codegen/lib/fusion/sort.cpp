    ...
    for (int j{0}; j < vstride; j++) {
      ...
      if (vwork.size(0) != 0) {
        ...
        for (int c_k{0}; c_k < n; c_k++) {
          if (std::isnan(vwork[c_k])) {
            iidx_tmp = (n - bLen) - 1;
            iidx[iidx_tmp] = c_k + 1;
            xwork[iidx_tmp] = vwork[c_k];
            bLen++;
          } else {
            dim++;
            idx4[dim - 1] = c_k + 1;
...
void sort(double x[16], int idx[16])
{
  ...
  nNaNs = 0;
  ib = 0;
  for (int c_k{0}; c_k < 16; c_k++) {
    if (std::isnan(x[c_k])) {
      idx[15 - nNaNs] = c_k + 1;
      xwork[15 - nNaNs] = x[c_k];
      nNaNs++;
    } else {
      ib++;
      idx4[ib - 1] = static_cast<signed char>(c_k + 1);
      x4[ib - 1] = x[c_k];
      if (ib == 4) {
        double d;
        double d1;
        ...
        if (d1 <= d) {
          ...
          if (d1 <= d) {
            ...
            i2 = static_cast<signed char>(i3);
            b_i3 = static_cast<signed char>(i4);
          } else if (d1 <= x4[i4 - 1]) {
            i = static_cast<signed char>(i1);
            b_i1 = static_cast<signed char>(i3);
            i2 = static_cast<signed char>(ib);
            b_i3 = static_cast<signed char>(i4);
          } else {
            i = static_cast<signed char>(i1);
            b_i1 = static_cast<signed char>(i3);
            i2 = static_cast<signed char>(i4);
  ...
  if (ib > 0) {
    ...
    for (int c_k{0}; c_k < quartetOffset; c_k++) {
      i1 = perm[c_k] - 1;
      i3 = ((c_k - nNaNs) - ib) + 16;
      idx[i3] = idx4[i1];
      x[i3] = x4[i1];
  ...
  ib = (nNaNs >> 1) + 16;
  for (int c_k{0}; c_k <= ib - 17; c_k++) {
    i1 = (c_k - nNaNs) + 16;
    quartetOffset = idx[i1];
    idx[i1] = idx[15 - c_k];
    idx[15 - c_k] = quartetOffset;
    x[i1] = xwork[15 - c_k];
    x[15 - c_k] = xwork[i1];
  }
  if ((nNaNs & 1) != 0) {
  ...
  if (16 - nNaNs > 1) {
    ...
    while (i3 > 1) {
      ...
      for (int c_k{0}; c_k < i3; c_k++) {
        merge(idx, x, c_k * ib, quartetOffset, quartetOffset, iwork, xwork);
      }
      quartetOffset = ib;
    }
    if (16 - nNaNs > quartetOffset) {
      merge(idx, x, 0, quartetOffset, 16 - (nNaNs + quartetOffset), iwork,
            xwork);
    }
  }
}
