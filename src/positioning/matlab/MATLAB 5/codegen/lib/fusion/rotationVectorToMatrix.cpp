                            ...
                            double rotationMatrix[9])
{
  ...
  if (theta < 1.0E-6) {
    ...
    for (int c_k{0}; c_k < 3; c_k++) {
      b_a[c_k + 3 * c_k] = 1;
      b_u[3 * c_k] = u[0] * u[c_k];
      b_u[3 * c_k + 1] = u[1] * u[c_k];
      b_u[3 * c_k + 2] = u[2] * u[c_k];
    }
    for (int c_k{0}; c_k < 9; c_k++) {
      a[c_k] = (static_cast<double>(b_a[c_k]) * absxk + a[c_k]) +
               (1.0 - absxk) * b_u[c_k];
    }
  }
  for (int c_k{0}; c_k < 3; c_k++) {
    rotationMatrix[3 * c_k] = a[c_k];
    rotationMatrix[3 * c_k + 1] = a[c_k + 3];
    rotationMatrix[3 * c_k + 2] = a[c_k + 6];
  }
}
