...
%             used_psi_next = headingest_a_aver_v;
            ...
            if mod(headingest_a_aver_v,2*pi) - gyro_psi > pi
                headingest_a_aver_vm = mod(headingest_a_aver_v,2*pi)-2*pi;
            else
                headingest_a_aver_vm = mod(heading_est,2*pi);
            end
...
%             qua_acc = EulerToQuaternion(acc_b_phi, acc_b_theta, gyro_psi);
            qua_kf = EulerToQuaternion(kf_phi, kf_theta, gyro_psi);
