...
for k = 1 : 7577
    ...
    if min_stamp_i == 5
        ...
        IMUacc(end,:) = [IMUMsg{k0(5)}.LinearAcceleration.X IMUMsg{k0(5)}.LinearAcceleration.Y IMUMsg{k0(5)}.LinearAcceleration.Z]*1;
...
%                 UWB_LS_Pos(:,end) = [real(mean(tag_pos_est)) ;imag(mean(tag_pos_est)); zt_b] ;
            ...
            if mod(headingest_a_aver_v+pi,2*pi) - kf_psi > pi
