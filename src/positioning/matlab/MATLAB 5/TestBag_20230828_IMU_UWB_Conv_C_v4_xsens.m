...
aa = rosbag('[xsens]2023-12-06-15-29-37.bag');
...
while((length(UWBMsg{1})>=k0(1))&&(length(UWBMsg{3})>=k0(3))&&(length(UWBMsg{4})>=k0(4))&&(length(IMUMsg)>=k0(5)))
    
    k = k + 1;
    for lk = 1 : 4
        if length(UWBMsg{lk})>0
            s_time_a(lk) = double(UWBMsg{lk}{k0(lk)}.Header.Stamp.Sec)+double(UWBMsg{lk}{k0(lk)}.Header.Stamp.Nsec)/10^9;
        else
            s_time_a(lk) = 10000000000000000000;
        end
    end
