  ...
  for (npages = 0; npages < xi; npages++) {
    ...
    for (lastBlockLength = 0; lastBlockLength < 29; lastBlockLength++) {
      b_dv[lastBlockLength] =
          DistList[((static_cast<int>(tag_center_pos_est_re) +
    ...
    for (lastBlockLength = 0; lastBlockLength < 29; lastBlockLength++) {
      ...
      b_dv[lastBlockLength] =
          DistListTime[((static_cast<int>(tag_center_pos_est_re) +
                         DistListTime.size(0) *
  ...
  if (TagNum == 4.0) {
    ...
    if ((DistPredicted.size(0) == 0) || (DistPredicted.size(1) == 0)) {
      ...
      for (xi = 0; xi < npages; xi++) {
        ...
        for (c_k = 2; c_k <= firstBlockLength; c_k++) {
          y[xi] = y[xi] + DistPredicted[(xpageoffset + c_k) - 1];
        }
        for (int ib{2}; ib <= nblocks; ib++) {
          ...
          for (c_k = 2; c_k <= hi; c_k++) {
            bsum += DistPredicted[(xblockoffset + c_k) - 1];
          }
          y[xi] = y[xi] + bsum;
        }
      }
    ...
    if (y.size(1) <= 2) {
      ...
      if (!std::isnan(y[0])) {
        ...
        c_k = 2;
        exitg1 = false;
        while ((!exitg1) && (c_k <= firstBlockLength)) {
          if (!std::isnan(y[c_k - 1])) {
            npages = c_k;
            exitg1 = true;
          } else {
            c_k++;
      ...
      if (npages == 0) {
        ...
        bsum = y[npages - 1];
        xi = npages + 1;
        for (c_k = xi; c_k <= firstBlockLength; c_k++) {
          tag_center_pos_est_re = y[c_k - 1];
    ...
    if (bsum > 0.0) {
      ...
      PrevHeading = GetInitPos4(xain_data, xain_size, yain_data, yain_size,
                                DistPredicted, tag_pos_b, LnC, prevTagPos,
                                prevTagHeading, tag_pos_est_aver, ErrSum);
    ...
    firstBlockLength = Tag_Pos_List.size(2);
    for (xi = 0; xi < firstBlockLength; xi++) {
    ...
    if ((dc.re != 0.0) || (dc.im != 0.0)) {
      ...
      for (xi = 0; xi < firstBlockLength; xi++) {
        ...
        tag_center_pos_est_re =
            Tag_Pos_List[32 * xi + 11].re - Tag_Pos_List[32 * xi].re;
        MeanA_re = Tag_Pos_List[32 * xi + 11].im - Tag_Pos_List[32 * xi].im;
        if (MeanA_re == 0.0) {
          if (ai == 0.0) {
    ...
    tag_center_pos_est_re = PrevHeading - heading_est_a[58];
    ...
    tag_center_pos_est_im = std::round(PrevHeading - static_cast<double>(xi));
    ...
    bsum = std::round(PrevHeading + static_cast<double>(lastBlockLength));
    if (bsum < 2.147483648E+9) {
      if (bsum >= -2.147483648E+9) {
        lastBlockLength = static_cast<int>(bsum);
      } else {
    ...
    for (xi = 0; xi < 59; xi++) {
      centerest_a[xi] = centerest_a[xi + 1];
      centerest_a_aver[xi] = centerest_a_aver[xi + 1];
      centerest_a[xi + 60] = centerest_a[xi + 61];
      centerest_a_aver[xi + 60] = centerest_a_aver[xi + 61];
    ...
    if (r > 60.0) {
      ...
      double im;
      for (xi = 0; xi < 30; xi++) {
        tag_center_pos_est_re = centerest_a[xi + 60];
        b_x[xi].re = centerest_a[xi] + 0.0 * tag_center_pos_est_re;
        b_x[xi].im = tag_center_pos_est_re;
      }
      tag_center_pos_est_re = b_x[0].re;
      tag_center_pos_est_im = b_x[0].im;
      for (c_k = 0; c_k < 29; c_k++) {
        tag_center_pos_est_re += b_x[c_k + 1].re;
        tag_center_pos_est_im += b_x[c_k + 1].im;
      }
      ...
      for (xi = 0; xi < 30; xi++) {
        tag_center_pos_est_re = centerest_a[xi + 90];
        b_x[xi].re = centerest_a[xi + 30] + 0.0 * tag_center_pos_est_re;
        b_x[xi].im = tag_center_pos_est_re;
      }
      tag_center_pos_est_re = b_x[0].re;
      tag_center_pos_est_im = b_x[0].im;
      ...
      for (c_k = 0; c_k < 29; c_k++) {
        tag_center_pos_est_re += b_x[c_k + 1].re;
        tag_center_pos_est_im += b_x[c_k + 1].im;
        b_y += heading_est_a[c_k + 1];
        ar += heading_est_a[c_k + 31];
      }
      if (tag_center_pos_est_im == 0.0) {
      ...
      centerest_a_aver[59] = ai + tag_center_pos_est_re;
      if (tag_center_pos_est_im == 0.0) {
        tag_center_pos_est_im = 0.0;
      } else {
        tag_center_pos_est_im /= 2.0;
      }
      centerest_a_aver[119] = im + tag_center_pos_est_im;
      bsum = ar / 30.0;
    ...
    bsum = coder::b_mod(headingest_a_aver[59]);
    if (bsum * 0.0 == 0.0) {
