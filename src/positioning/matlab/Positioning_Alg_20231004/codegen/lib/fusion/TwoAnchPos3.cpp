                 ...
                 double Prob[2])
{
  ...
  for (int c_k{0}; c_k < 2; c_k++) {
    b_d_tmp_tmp = y_tmp[c_k + 2];
    d_tmp = y_tmp[c_k];
    b_y_tmp[c_k] = d_tmp * AA + b_d_tmp_tmp * b_d;
    b_y_tmp[c_k + 2] = d_tmp * -s + b_d_tmp_tmp * d_tmp_tmp;
  }
  ...
  for (int c_k{0}; c_k < 2; c_k++) {
    b_d_tmp_tmp = A[c_k + 2];
    d_tmp = A[c_k];
    X1[c_k] = (d_tmp * AA + b_d_tmp_tmp * -s) * d_idx_0 +
  ...
  for (int c_k{0}; c_k < 2; c_k++) {
    b_d_tmp_tmp = A[c_k + 2];
    d_tmp = A[c_k];
    X2[c_k] = (d_tmp * AA + b_d_tmp_tmp * -s) * d_idx_0 +
              (d_tmp * b_d + b_d_tmp_tmp * d_tmp_tmp) * d_idx_1_tmp;
  }
  AA = X1[0] + 0.0 * X1[1];
  s = X1[1];
  ...
  for (int c_k{0}; c_k < nx; c_k++) {
    tmp_data[c_k] =
        rt_hypotd_snf(anch_pos_data[c_k].re - AA, anch_pos_data[c_k].im - s);
  }
  if (dist_a.size(0) == anch_pos_size[1]) {
    x.set_size(1, anch_pos_size[1]);
    nx = anch_pos_size[1];
    for (int c_k{0}; c_k < nx; c_k++) {
      x[c_k] = tmp_data[c_k] - dist_a[c_k];
    }
  } else {
