                   ...
                   double Pos[2], double Prob[4])
{
  ...
  for (b_k = 0; b_k < 3; b_k++) {
    ...
    A[b_k + 3] = d3;
    x_tmp = dist[b_k];
    y[b_k] = ((x_tmp * x_tmp - dist[3] * dist[3]) - (d * d - xa[3] * xa[3])) -
             (d2 * d2 - ya[3] * ya[3]);
    xpageoffset = b_k << 1;
  ...
  for (int i{0}; i < 2; i++) {
    ...
    for (b_k = 0; b_k < 2; b_k++) {
      b_y_tmp[i + (b_k << 1)] =
          (d * A[3 * b_k] + d1 * A[3 * b_k + 1]) + d2 * A[3 * b_k + 2];
    }
  }
  ...
  for (int i{0}; i < 2; i++) {
    ...
    for (b_k = 0; b_k < 3; b_k++) {
      xpageoffset = b_k << 1;
      d += (d1 * y_tmp[xpageoffset] + d2 * y_tmp[xpageoffset + 1]) * y[b_k];
    }
    Pos[i] = d;
  }
  d = xa[0] - Pos[0];
                     ...
                     const double dist[3], double Pos[2], double Prob[3])
{
  ...
  for (int b_k{0}; b_k < 2; b_k++) {
    d3 = y_tmp[b_k + 2];
    d4 = y_tmp[b_k];
    b_y_tmp[b_k] = d4 * b_y_idx_0_tmp + d3 * d;
    b_y_tmp[b_k + 2] = d4 * d1 + d3 * d2;
  }
  coder::b_inv(b_y_tmp, A);
  ...
  for (int b_k{0}; b_k < 3; b_k++) {
    y_idx_1 = y[b_k] + b_y[b_k];
    x[3 * b_k] = y_idx_1 - c_y_idx_0_tmp;
    x[3 * b_k + 1] = y_idx_1 - y_idx_1_tmp;
    x[3 * b_k + 2] = y_idx_1 - y_idx_0_tmp;
  }
  for (int b_k{0}; b_k < 9; b_k++) {
    c_y[b_k] = std::abs(x[b_k]);
  }
  for (int b_k{0}; b_k < 3; b_k++) {
    int xpageoffset;
    xpageoffset = b_k * 3;
