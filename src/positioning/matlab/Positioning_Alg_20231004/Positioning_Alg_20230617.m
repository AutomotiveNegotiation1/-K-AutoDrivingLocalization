...
for r = 1 : length(x)
    ...
    if r > InitLeng
        dist = dist_a;
        dist(3,1:4) = dist_a(3,1:4)+1;
        %         dist = dist_a;
    else
        dist = dist_a;
        dist(2,2:4) = dist_a(2,2:4)+1;
        ...
        for MM = NN+1 : Ln
            if NN~=MM
                ...
                for PP = 1 : Lp
                    ...
                    [Pos2{PP},Prob2{PP}]=TwoAnchPos3([xa(NN) xa(MM)], [ya(NN) ya(MM)], [dist(NN,PP) dist(MM,PP)],[real(tag_pos_est(PP)) imag(tag_pos_est(PP))],[Xt_c_e Yt_c_e],anch_pos,dist(:,PP));
                    if (PP > 1)&&(PP<Lp)
                        Heading_T(PP-1) = ((Pos2{PP}(1,1)+j*Pos2{PP}(1,2))-(Pos2{PP-1}(1,1)+j*Pos2{PP-1}(1,2)))/((xt_b(PP)+j*yt_b(PP))-(xt_b(PP-1)+j*yt_b(PP-1)));
                    elseif PP == Lp
                        Heading_T(PP-1) = ((Pos2{PP}(1,1)+j*Pos2{PP}(1,2))-(Pos2{PP-1}(1,1)+j*Pos2{PP-1}(1,2)))/((xt_b(PP)+j*yt_b(PP))-(xt_b(PP-1)+j*yt_b(PP-1)));
                        Heading_T(PP) = ((Pos2{1}(1,1)+j*Pos2{1}(1,2))-(Pos2{PP}(1,1)+j*Pos2{PP}(1,2)))/((xt_b(1)+j*yt_b(1))-(xt_b(PP)+j*yt_b(PP)));
                    end

                end
%                 ttt = Pos2{2}(1,2)*Pos2{3}(1,2)-Pos2{1}(1,2)*Pos2{2}(1,2)-Pos2{1}(1,2)*Pos2{3}(1,2)+Pos2{1}(1,2)*Pos2{1}(1,2) + Pos2{2}(1,1)*Pos2{3}(1,1)-Pos2{1}(1,1)*Pos2{2}(1,1)-Pos2{1}(1,1)*Pos2{3}(1,1)+Pos2{1}(1,1)*Pos2{1}(1,1)

                H_INdex(QS,:) = [NN,MM];
                Heading_Total(QS,:) = Heading_T;
                QS = QS + 1;
    ...
    %         AB(1) = sum(sum(abs(KK(A1,[2 3 4]))))+sum(KK(A1,1));
    %         AB(2) = sum(sum(abs(KK(A2,[1 3 4]))))+sum(KK(A2,2));
    %         AB(3) = sum(sum(abs(KK(A3,[1 2 4]))))+sum(KK(A3,3));
    %         AB(4) = sum(sum(abs(KK(A4,[1 2 3]))))+sum(KK(A4,4));


    AB(1) = sum(sum(abs(KK(A1,[2 3 4]))));
    AB(2) = sum(sum(abs(KK(A2,[1 3 4]))));
    AB(3) = sum(sum(abs(KK(A3,[1 2 4]))));
    AB(4) = sum(sum(abs(KK(A4,[1 2 3]))));
