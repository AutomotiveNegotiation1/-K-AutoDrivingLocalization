...
aa = rosbag('20230807_driving_record.bag');
...
for k = 1 : 3435
    ...
    s_time_a = double(IMUMsg{k0(5)}.Header.Stamp.Sec)+double(IMUMsg{k0(5)}.Header.Stamp.Nsec)/10^9;
    ...
    if min_stamp_i == 1
        ...
        IMUacc(end,:) = [IMUMsg{k0(5)}.LinearAcceleration.X IMUMsg{k0(5)}.LinearAcceleration.Y IMUMsg{k0(5)}.LinearAcceleration.Z];
        IMUgyro(1:end-1, :) = IMUgyro(2:end, :);
        IMUgyro(end,:)  = [IMUMsg{k0(5)}.AngularVelocity.X IMUMsg{k0(5)}.AngularVelocity.Y IMUMsg{k0(5)}.AngularVelocity.Z];
%         dt = 0.03;
%         IMUgyro = IMUgyro/40;
%         IMUacc(1:2) = IMUacc(1:2)/40;
%         dt_acc = 0.03;
...
%             used_psi_next = headingest_a_aver_v;
            ...
            Acc_Acc(:,end) = rotationVectorToMatrix([0;0;used_psi])'*([acc_b(1);acc_b(2);0]);
...
figure(232);hold off;plot(-Pos(2,:),-Pos(1,:),'.')
