...
aa = rosbag('20230809_driving_record_2.bag');
...
%         RxDistTot(k,RxID) = RxDist;
        ...
        if Nanchor > 1
            [UWBout] = UWBpos2(Ln, Lp, LnC, PP, Nanchor, RxIDUWB, RxDistUWB, s_time, tag_pos_b, xain, yain);
            %             UWBoutTot(k,:) = UWBout;
...
%             [tag_pos_est, heading_est] = UWBposEnh(LnC, Lp, PP, Nanchor, RxID, RxDist, s_time, tag_pos_b, xa, ya, tag_pos_est, heading_est);
            ...
            if ((k0(5)>50))  % INS/UWB
                ...
                if (kalman_on == 1) && (k < 1000)%&& (Nanchor>2) && (Reliability >2)
                    ...
                    [kf_phi,kf_theta,kf_psi] = EulerKalman_2(qua_acc,AA,qua_kf);
                    kf_phi =real(kf_phi);
                ...
                if (kalman_on == 1) && (k < 1000)%&& (Nanchor>2)
                    R = 1e-0*eye(6);
...
figure(99999);hold off;plot(TagPosTime,RxDistOrigTot(:,1,1),'bo');hold on;plot(TagPosTime,RxDistOrigTot(:,2,1),'go');plot(TagPosTime,RxDistOrigTot(:,3,1),'ro');plot(TagPosTime,RxDistOrigTot(:,4,1),'ko');plot(TagPosTime,real(mean(TagUWBPosTot,2)),'r')
