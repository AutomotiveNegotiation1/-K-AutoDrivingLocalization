...
IMUData.IMUAcc = [0 0 0]; %% IMUAcc
IMUData.IMUgyro = [0 0 0]; %% IMUgyro
IMUData.s_time = 0;       %% s_time
...
while((length(UWBMsg{1})>=k0(1))&&(length(UWBMsg{2})>=k0(2))&&(length(UWBMsg{3})>=k0(3))&&(length(UWBMsg{4})>=k0(4))&&(length(IMUMsg)>=k0(5)))
    ...
    if min_stamp_i == 5
        if IMUSel == 1
            IMUData.IMUAcc  = [IMUAMsg{k0(5)}.Vector.X IMUAMsg{k0(5)}.Vector.Y IMUAMsg{k0(5)}.Vector.Z];
            IMUData.IMUgyro = [IMUVMsg{k0(5)}.Vector.X IMUVMsg{k0(5)}.Vector.Y IMUVMsg{k0(5)}.Vector.Z];
        else
            IMUData.IMUAcc = [IMUMsg{k0(5)}.LinearAcceleration.X IMUMsg{k0(5)}.LinearAcceleration.Y IMUMsg{k0(5)}.LinearAcceleration.Z]*1;
            IMUData.IMUgyro  = [IMUMsg{k0(5)}.AngularVelocity.X IMUMsg{k0(5)}.AngularVelocity.Y IMUMsg{k0(5)}.AngularVelocity.Z];
        end
        IMUData.s_time = s_time;
   %     [cent_pos_est, cent_vel_est, kf_phi, kf_theta, kf_psi, gyro_psi, b_acc_o, b_gyro, state_IMU, acc_b_phi, acc_b_theta ] = IMUposTest(IMUacc_c, IMUgyro_c, s_time, b_acc_o, b_gyro, IMUSel, kf_phi, kf_theta, kf_psi, gyro_psi, cent_pos_est, cent_vel_est);
        [cent_pos_est, cent_vel_est, kf_psi, gyro_psi, b_acc_o, b_gyro, state_IMU, acc_b_phi, acc_b_theta ] = IMUpos(IMUData.IMUAcc, IMUData.IMUgyro, IMUData.s_time, b_acc_o, b_gyro, IMUSel, kf_psi, gyro_psi, cent_pos_est, cent_vel_est);

        k0(5) = k0(5) + 1;

    elseif min_stamp_i<5

        UWBData.s_time = s_time;
        UWBData.tagNum = min_stamp_i;
        ...
        UWBData.RxID = RxIDUWB;
        UWBData.RxDist = RxDistUWB;
        UWBData.AnchPos = xain+j*yain;
        UWBData.Nanchor = Nanchor;
