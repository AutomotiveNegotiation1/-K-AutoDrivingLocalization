...
while((length(UWBMsg{1})>=k0(1))&&(length(UWBMsg{2})>=k0(2))&&(length(UWBMsg{3})>=k0(3))&&(length(UWBMsg{4})>=k0(4))&&(length(IMUMsg)>=k0(5)))
    ...
    elseif min_stamp_i<5
        ...
        if (Nanchor > 1) && (isempty(find(RxDistUWB(1:Nanchor)==0, 1)))
            ...
            [cent_pos_est, cent_vel_est, kf_psi, IMUposU, acc_b_phi, tag_center_vel_est] = fusion(tag_center_vel_est, state_IMU, Nanchor, b_acc_o, ...
                acc_b_theta, acc_b_phi, UWBErrSum, init_flag, kalman_on, k0 ,cent_pos_est, cent_vel_est, kf_psi, tag_pos_est, ...
                heading_est, zt_b);
        else
            k0(PP) = k0(PP) + 1;
