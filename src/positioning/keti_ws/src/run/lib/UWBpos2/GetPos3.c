  ...
  for (L1 = 0; L1 < i; L1++) {
    ...
    for (L2 = 0; L2 < i1; L2++) {
      ...
      for (nd2 = 0; nd2 < i2; nd2++) {
        ...
        xa_size[0] = 1;
        xa_size[1] = RxID_size;
        for (i3 = 0; i3 < RxID_size; i3++) {
          i4 = (int)RxID_data[i3] - 1;
          Pos2C_re = ya_data[i4];
          b_xa_data[i3].re = xa_data[i4] + 0.0 * Pos2C_re;
          b_xa_data[i3].im = Pos2C_re;
        }
        c_xa_data.data = &b_xa_data[0];
        c_xa_data.size = &xa_size[0];
        c_xa_data.allocatedSize = 6;
        c_xa_data.numDimensions = 2;
        c_xa_data.canFreeData = false;
        ...
        TwoAnchPos3(Est_F, Est_H, dist, &c_xa_data, &b_dist_data, Pos2, Prob2);
        Pos2C_data[(int)b_r - 1].re = Pos2[0];
        Pos2C_data[(int)b_r - 1].im = Pos2[2];
        Pos2C_data[((int)b_r + Pos2C->size[0]) - 1].re = Pos2[1];
        Pos2C_data[((int)b_r + Pos2C->size[0]) - 1].im = Pos2[3];
      }
      ...
      for (Qn = 0; Qn < 2; Qn++) {
        ...
        for (P = 0; P < i5; P++) {
          ...
          b_r = im + (b_r * Est_H_p_im + brm * Est_H_p_re);
          Est_P_data[P].re = br;
          Est_P_data[P].im = b_r;
          nd2 = (int)ia_data[P] - 1;
          b_r = rt_hypotd_snf(br - Pos2C_data[P + Pos2C->size[0] * nd2].re,
                              b_r - Pos2C_data[P + Pos2C->size[0] * nd2].im);
          Pos2C_re += b_r * b_r;
        ...
        for (P = 0; P < i6; P++) {
          ...
          if (j2 == i7) {
            ...
            for (i7 = 0; i7 < j2; i7++) {
              ...
              b_r = ya_data[nd2];
              anch_pos_data[i7].re = xa_data[nd2] + 0.0 * b_r;
              anch_pos_data[i7].im = b_r;
            }
          } else {
            c_binary_expand_op(anch_pos_data, anch_pos_size, xa_data,
                               b_RxIDprev, indp, j2 - 1, ya_data, i7 - 1);
          }
          j2 = (int)b_RxIDprevLen[indp];
          if (j2 - 1 >= 0) {
          ...
          sort(Est_Ft_data, &j2);
          if ((b_RxIDprevLen[indp] >= 2.0) || rtIsNaN(b_RxIDprevLen[indp])) {
            b_r = 2.0;
          } else {
            b_r = b_RxIDprevLen[indp];
          }
          if (b_r < 1.0) {
            nd2 = 0;
          } else {
