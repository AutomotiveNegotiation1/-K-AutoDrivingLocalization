                 ...
                 int Pos2_size[2], creal_T Pos3_data[], int Pos3_size[2])
{
  ...
  if (Nanchor == 2.0) {
    ...
    for (k = 0; k < i; k++) {
      ...
      if (loop_ub - 1 >= 0) {
        x[0] = (RxDistin_data[k] != 0.0);
      }
      for (b_loop_ub = 0; b_loop_ub < loop_ub; b_loop_ub++) {
        boolean_T exitg1;
        boolean_T y;
        boffset = (k + b_loop_ub) + 1;
        x[1] = (RxDistin_data[boffset] != 0.0);
        y = true;
        coffset = 0;
        exitg1 = false;
    ...
    for (i = 0; i < 12; i++) {
      for (loop_ub = 0; loop_ub < 12; loop_ub++) {
        coffset = loop_ub + 12 * i;
        x_data[coffset].re = Pos3_data[i].re - Pos3_data[loop_ub].re;
        x_data[coffset].im = Pos3_data[i].im - Pos3_data[loop_ub].im;
      }
    }
    for (k = 0; k < 144; k++) {
      bkj = rt_hypotd_snf(x_data[k].re, x_data[k].im);
      b_y_data[k] = bkj * bkj;
    }
