...
// Return Type  : void
//
void IMUpos(const double IMUacc_c[3], const double IMUgyro_c[3], double,
            ...
            double *state_o, double *acc_b_phi, double *acc_b_theta)
{
  ...
  if (k > 50.0) {
    ...
    for (i = 0; i < 19; i++) {
      IMUgyro_tmp = 3 * (i + 1);
      Acc_Pos[3 * i] = Acc_Pos[IMUgyro_tmp];
      Acc_Vel[3 * i] = Acc_Vel[IMUgyro_tmp];
      Acc_Acc[3 * i] = Acc_Acc[IMUgyro_tmp];
      Acc_Vel_tmp = 3 * i + 1;
      Acc_Pos[Acc_Vel_tmp] = Acc_Pos[IMUgyro_tmp + 1];
      Acc_Vel[Acc_Vel_tmp] = Acc_Vel[IMUgyro_tmp + 1];
      Acc_Acc[Acc_Vel_tmp] = Acc_Acc[IMUgyro_tmp + 1];
      Acc_Vel_tmp = 3 * i + 2;
      Acc_Pos[Acc_Vel_tmp] = Acc_Pos[IMUgyro_tmp + 2];
      Acc_Vel[Acc_Vel_tmp] = Acc_Vel[IMUgyro_tmp + 2];
    ...
    *acc_b_theta = std::atan(
    ...
    EulerKalman_2(c_cosPhi, b_I, d_cosPhi, &NextEuler_idx_2, &UpdateMat_tmp,
                  kf_psi);
    ...
    for (Acc_Vel_tmp = 0; Acc_Vel_tmp < 3; Acc_Vel_tmp++) {
      ...
      b_gyro[Acc_Vel_tmp] = IMUgyro[IMUgyro_tmp];
      b_acc_o[Acc_Vel_tmp] = IMUacc[IMUgyro_tmp];
      for (int b_k{0}; b_k < 19; b_k++) {
        i = (IMUgyro_tmp + b_k) + 1;
        b_gyro[Acc_Vel_tmp] += IMUgyro[i];
        b_acc_o[Acc_Vel_tmp] += IMUacc[i];
      }
      b_gyro[Acc_Vel_tmp] /= 20.0;
      b_acc_o[Acc_Vel_tmp] = b_acc_o[Acc_Vel_tmp] / 20.0 + b_dv[Acc_Vel_tmp];
      acc_b[Acc_Vel_tmp] = IMUacc[20 * Acc_Vel_tmp + 19] - b_acc_o[Acc_Vel_tmp];
    }
    *acc_b_phi = std::atan(acc_b[1] / acc_b[2]);
    *acc_b_theta = std::atan(
        acc_b[0] / std::sqrt(acc_b[1] * acc_b[1] + acc_b[2] * acc_b[2]));
...
void IMUpos_init()
{
  ...
  std::memset(&Acc_Vel[0], 0, 60U * sizeof(double));
  std::memset(&Acc_Pos[0], 0, 60U * sizeof(double));
  // %%%% For Test %%%%%%
