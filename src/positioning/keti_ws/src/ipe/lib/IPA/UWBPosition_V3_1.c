 ...
 * UWBPosition_V3_1.c
 *
 * Code generation for function 'UWBPosition_V3_1'
 *
 */
...
double UWBPosition_V3_1(double s_time, double Ln, double Nanchor, double TagID,
                        ...
                        double zt_b, creal_T *PosUWBO, double *UWBFull)
{
  ...
  if (RxIDUWB_size[1] > 1) {
    if (Nanchor != 0.0) {
      /*          for kk = 1 : length(RxIDUWB) */
      /*              TempDist =
       * squeeze(DistMap(TagID,RxIDUWB(kk),end-10:end-1)); */
      /*              ListNZDist = find(TempDist~=0); */
      /*   */
      /*              if length(ListNZDist)>0 */
      /*                  meanPrev = mean(TempDist(ListNZDist)); */
      /*                  RxDistT(kk) = (meanPrev+RxDist(kk))/2; */
      /*              else */
      /*                  RxDistT(kk) = RxDist(kk); */
      /*              end */
      /*          end */
      /*  [Pos1, Pos2, Pos3] = UWBpos_V2_3(Nanchor, RxIDUWB, RxDistT, xain,
       * yain); */
  ...
  if (gs > 3.0) {
    ...
    HeadingUWBO = UWBMultiTagPos_V3_1(PosUWB2, xt_b_data, xt_b_size, yt_b_data,
                                      yt_b_size, xain_data, xain_size,
                                      yain_data, yain_size, DistMap, PosUWBO);
...
void UWBPosition_V3_1_free(void)
{
  emxFree_real_T(&DistMap);
}

void UWBPosition_V3_1_init(void)
{
  emxInitMatrix_cell_wrap_0(PosUWB2);
  emxInit_real_T(&DistMap);
...
/* End of code generation (UWBPosition_V3_1.c) */
