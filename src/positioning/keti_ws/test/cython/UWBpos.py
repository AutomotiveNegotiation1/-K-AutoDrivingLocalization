...
def UWBpos(Ln, Lp, TagNum, Nanchor, RxID, RxDist, s_time, tag_pos_b, xa, ya):
    ...
    if r < 10 * Lp:
        ...
        IndT = np.where(np.sum(DistT, axis=1) < 100000)
        DistT = DistT[IndT]
        AnchID = IndT
        ...
        for PPC in range(Lp):
            InterpPosition[PPC, 0] = InterpPos(Tag_Pos_List[:, 0, PPC], np.real(Tag_Pos_List[:, 2, PPC]), s_time)
            InterpPosition[PPC, 1] = InterpPos(Tag_Pos_List[:, 0, PPC], np.imag(Tag_Pos_List[:, 2, PPC]), s_time)
